...

{
    programs.bash = {
        ...
        # --------------------------
        # Nix Channel - CHECK & OR switch UNSTABLE
        # --------------------------
        # Check the nix channel from the stable channel
        alias noss-channel-check='cd ~/.nix-config/nixos-25.05/ && sudo nix-channel --list'
        # Check the nix channel from the unstable channel
        alias nosu-channel-check='cd ~/.nix-config/nixos-unstable/ && sudo nix-channel --list'
